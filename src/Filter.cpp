...
static llvm::cl::opt<std::string> FilterFile("filter-file", llvm::cl::Hidden,
                                             llvm::cl::desc("Path to function filter file"));

void FunctionFilterPass::getAnalysisUsage(llvm::AnalysisUsage& AU) const { AU.setPreservesAll(); }
...
void FunctionFilterPass::loadFile(llvm::Module& M, std::string file_name) {
  ...
  for (auto& F : M) {
    ...
    if (function_names.find(demangled_name) != function_names.end() ||
        function_names.find(F.getName()) != function_names.end()) {
...
FunctionInformation* FunctionFilterPass::get_functions_info() { return &(this->m_functions_info); }
static llvm::RegisterPass<FunctionFilterPass> X("filter-func",
                                                "Include functions in a given file in any transformation");